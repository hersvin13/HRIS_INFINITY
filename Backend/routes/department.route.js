...
router.route('/totalEmployees').get(async (req, res) => {
  try {
    const totalEmployees = await Department.findAll({
      attributes: [
        'col_id',
        [sequelize.fn('COUNT', sequelize.col('employees.col_id')), 'totalEmployees'],
      ],
      include: {
    ...
    totalEmployees.forEach((dept) => {
      totalEmployeesMap[dept.getDataValue('col_id')] = dept.getDataValue('totalEmployees');
    });
...
router.route('/deptADD').post(async (req, res) => {
    ...
    try {
      const department = await Department.create({ col_departmentName: departmentName });
